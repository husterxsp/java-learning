...
### 尝试反编译switch、String的“+”、lambda表达式、java 10的本地变量推断等。
...
需要注意的是 switch(string) 支持的新语法，具体可以参看上面的反编译代码。其实也只是将 switch(string) 转换为 switch(string.hashCode())，hashCode()返回的是 int。
...
我们知道java string对象是不可变的，那么为了提高性能，如果新的string对象已经创建过，那么会将该对象直接指向已经创建的对象。???
